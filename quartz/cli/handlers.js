...
`,
  ...
  if (template && template !== "default") {
    ...
    console.log(
      styleText(
        "green",
        `Created quartz.config.yaml from '${template}' template`,
      ),
    );
  } else {
...
export async function handleUpgrade(argv) {
      ...
      try {
        ...
        if (remaining.length === 0) {
          ...
          console.log(
            styleText(
              "cyan",
              "Resolved quartz.lock.json merge conflict automatically.",
            ),
          );
        }
      } catch {
  ...
  if (newVersion !== version) {
    console.log(
      styleText("cyan", `Upgraded Quartz: v${version} → v${newVersion}`),
    );
  } else {
    console.log(
      styleText("gray", `Quartz is already up to date (v${version})`),
    );
  }
